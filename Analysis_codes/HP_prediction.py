...
base_path = "/Path/to/HP_SNPs_ADSP_ADRC"
...
UKB_raw = pd.read_csv("/Path/to/HP/UKB_HP1_imputation.raw", delim_whitespace=True).drop(columns=metadata_cols)
f05_raw = pd.read_csv("/Path/to/HP/f05_genoEUR/f05_genoEUR_HPprotGWAS.raw", delim_whitespace=True).drop(columns=metadata_cols)
...
bim_eur = pd.read_csv(f"{base_path}/HP_haplotype_variant_PLINK_file_3506_EUR_samples.bim", delim_whitespace=True, header=None,
                      names=["CHR", "SNP", "CM", "POS", "A1", "A2"])
bim_UKB = pd.read_csv("/Path/to/HP/UKB_HP1_imputation.bim", delim_whitespace=True, header=None,
                      names=["CHR", "SNP", "CM", "POS", "A1", "A2"])


bim_f05 = pd.read_csv("/Path/to/HP/f05_genoEUR/f05_genoEUR_HPprotGWAS.bim", delim_whitespace=True, header=None,
                      names=["CHR", "SNP", "CM", "POS", "A1", "A2"])
...
predict_and_save(
    model,
    UKB_raw,
    "/Path/to/UKB_HP1_imputation.fam",
...
predict_and_save(
    model,
    f05_raw,
    "/Path/to/f05_genoEUR_HPprotGWAS.fam",
