sQTL_colocalization <- function(merged, all_suggestive_snps, name_dset, LC_dir, LC_threshold) {
  ...
  if (nrow(colocalizing_genes) > 0) {
    ...
    for (gene_row in 1:nrow(colocalizing_genes)) {
      ...
      fold_w <- "/storage1/fs1/belloy/Active/05_Projects/noahc/opt_QTL/Xchr_LD/sQTL/"
      snp_file <- paste0(fold_w, study, "_", tissue, "_sQTL_", discovery, "_", locus_index, ".txt")
      fwrite(snps_for_ld, snp_file, sep = "\t", col.names = FALSE)
      
      
      file_ld <- paste("LD_matrix_", study, tissue, "sQTL", discovery, locus_index, sep = "_")
