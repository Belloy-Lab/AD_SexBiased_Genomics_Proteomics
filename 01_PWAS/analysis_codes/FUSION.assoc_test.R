...
allele.qc = function(a1,a2,ref1,ref2) {	#take 4 arguments as a1, a2, ref1, ref2
        a1 = toupper(a1)	#convert alleles to uppercase
        a2 = toupper(a2)
        ref1 = toupper(ref1)
        ref2 = toupper(ref2)

	ref = ref1	#create a flip dictionary based on ref1 and ref2 for the complementary base
	flip = ref
	flip[ref == "A"] = "T"
	flip[ref == "T"] = "A"
	flip[ref == "G"] = "C"
	...
	snp = list()	#allele comparison = check if a1 and a2 are valid and not ambiguous (not equal to A T G or C), if ambiguous do not keep
	snp[["keep"]] = !((a1=="A" & a2=="T") | (a1=="T" & a2=="A") | (a1=="C" & a2=="G") | (a1=="G" & a2=="C"))
	snp[["keep"]][ a1 != "A" & a1 != "T" & a1 != "G" & a1 != "C" ] = F
	snp[["keep"]][ a2 != "A" & a2 != "T" & a2 != "G" & a2 != "C" ] = F
	snp[["flip"]] = (a1 == ref2 & a2 == ref1) | (a1 == flip2 & a2 == flip1)	#check if a1 or a2 needs to be flipped based on the ref flip dictionary
...
wgtlist = read.table(opt$weights,head=T,as.is=T)	# pos file listing molecular weight RDat files (must have columns WGT,ID,CHR,P0,P1)
wgtlist = wgtlist[ as.character(wgtlist$CHR) == as.character(opt$chr) , ]	#consider CHR/chr as character
chr = unique(wgtlist$CHR)	#list of unique CHR from wgtlist

N = nrow(wgtlist)	#N = number of rows in wgtlist; then create a table with the following column names of N length
...
genos = read_plink(paste(opt$ref_ld_chr,chr,sep=''),impute="avg")	#read plink files for each chr

# Match summary data to input, record NA where summary data is missing
m = match( genos$bim[,2] , sumstat$SNP )	#match genotypes between LD ref and sumstats
sum.missing = is.na(m)	#
sumstat = sumstat[m,]
...
## For each wgt file:
for ( w in 1:nrow(wgtlist) ) {	#loop through rows in wgtlist
	#cat( unlist(wgtlist[w,]) , '\n' )
	# Load weights
	wgt.file = paste(opt$weights_dir,"/",wgtlist$WGT[w],sep='')	#paste wgt directory/ each WGT in wgtlist
	load(wgt.file)	#load weight
	# Remove NAs (these should not be here)
	wgt.matrix[is.na(wgt.matrix)] = 0	#replace NA with 0 in weight
	
	# Match up the SNPs and weights
	m = match( snps[,2] , genos$bim[,2] )	#match snp ID between weight and LD ref bim
	m.keep = !is.na(m)	#indicate non NA SNP matches
	snps = snps[m.keep,]
	wgt.matrix = wgt.matrix[m.keep,,drop=F]	#subset wgt matrix for non NA SNP matches
	cur.genos = scale(genos$bed[,m[m.keep]])	#subset LD ref bed file to non NA SNP matches and scale/standardize by substracting mean and dividing by the standard deviation
	cur.bim = genos$bim[m[m.keep],]	#subset the LD bim file to non NA SNP matches
	# Flip WEIGHTS for mismatching alleles
	qc = allele.qc( snps[,5] , snps[,6] , cur.bim[,5] , cur.bim[,6] )	# perform the allele.qc as defined above for wgt snps as a1 and a2 to match bim alleles as ref1 and ref2
	wgt.matrix[qc$flip,] = -1 * wgt.matrix[qc$flip,]	#flip weights in wgt.matrix for mismatching alleles
