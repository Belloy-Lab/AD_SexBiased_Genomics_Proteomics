## make tables of non-sex-strat and sex-strat NGI CSF AD PWAS sex-specific genes including GWAS stats (beta, p, sex-heterogeneity)
...
f_PWAS = fread(paste0(file.path(PD_dir, f_dir, f_f), "sex-strat_NGI-CSF_weights.txt")); f_PWAS = as.data.frame(f_PWAS); print("Female Primary Discovery loaded")
m_PWAS = fread(paste0(file.path(PD_dir, m_dir, m_f), "sex-strat_NGI-CSF_weights.txt")); m_PWAS = as.data.frame(m_PWAS); print("Male Primary Discovery loaded")
f_SD_PWAS = fread(paste0(file.path(SD_dir, f_dir, f_f), "NGI-CSF_weights.txt")); f_SD_PWAS = as.data.frame(f_SD_PWAS); print("Female Secondary Discovery loaded")
m_SD_PWAS = fread(paste0(file.path(SD_dir, m_dir, m_f), "NGI-CSF_weights.txt")); m_SD_PWAS = as.data.frame(m_SD_PWAS); print("Male Secondary Discovery loaded")
...
a$Female.PD.GWAS.Beta = ifelse(a$Female.PD.GWAS.SNP %in% f_stats$SNP, f_stats$BETA[match(a$Female.PD.GWAS.SNP, f_stats$SNP)], NA)
a$Female.PD.GWAS.SE = ifelse(a$Female.PD.GWAS.SNP %in% f_stats$SNP, f_stats$SE[match(a$Female.PD.GWAS.SNP, f_stats$SNP)], NA)
a$Female.PD.GWAS.P = ifelse(a$Female.PD.GWAS.SNP %in% f_stats$SNP, f_stats$P[match(a$Female.PD.GWAS.SNP, f_stats$SNP)], NA)

a$Male.PD.GWAS.Beta = ifelse(a$Female.PD.GWAS.SNP %in% m_stats$SNP, m_stats$BETA[match(a$Female.PD.GWAS.SNP, m_stats$SNP)], NA)
a$Male.PD.GWAS.SE = ifelse(a$Female.PD.GWAS.SNP %in% m_stats$SNP, m_stats$SE[match(a$Female.PD.GWAS.SNP, m_stats$SNP)], NA)
a$Male.PD.GWAS.P = ifelse(a$Female.PD.GWAS.SNP %in% m_stats$SNP, m_stats$P[match(a$Female.PD.GWAS.SNP, m_stats$SNP)], NA)
...
a$Female.SD.GWAS.Beta = ifelse(a$Female.SD.GWAS.SNP %in% f_stats$SNP, f_stats$BETA[match(a$Female.SD.GWAS.SNP, f_stats$SNP)], NA)
a$Female.SD.GWAS.SE = ifelse(a$Female.SD.GWAS.SNP %in% f_stats$SNP, f_stats$SE[match(a$Female.SD.GWAS.SNP, f_stats$SNP)], NA)
a$Female.SD.GWAS.P = ifelse(a$Female.SD.GWAS.SNP %in% f_stats$SNP, f_stats$P[match(a$Female.SD.GWAS.SNP, f_stats$SNP)], NA)

a$Male.SD.GWAS.Beta = ifelse(a$Female.SD.GWAS.SNP %in% m_stats$SNP, m_stats$BETA[match(a$Female.SD.GWAS.SNP, m_stats$SNP)], NA)
a$Male.SD.GWAS.SE = ifelse(a$Female.SD.GWAS.SNP %in% m_stats$SNP, m_stats$SE[match(a$Female.SD.GWAS.SNP, m_stats$SNP)], NA)
a$Male.SD.GWAS.P = ifelse(a$Female.SD.GWAS.SNP %in% m_stats$SNP, m_stats$P[match(a$Female.SD.GWAS.SNP, m_stats$SNP)], NA)
...
fwrite(a, paste0(file.path(PD_dir, f_dir), "/", opt$file_name), na = NA, col.names = T, sep = '\t')
