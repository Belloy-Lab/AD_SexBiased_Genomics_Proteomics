## make tables of brain (non-sex-strat W23 and sex-strat W23) AD PWAS sex-specific genes including GWAS stats (beta, p, sex-heterogeneity)
...
f_PWAS = fread(paste0(file.path(PD_dir, f_dir, f_f), "Wingo23_weights.txt")); f_PWAS = as.data.frame(f_PWAS); print("Female Primary Discovery loaded")
m_PWAS = fread(paste0(file.path(PD_dir, m_dir, m_f), "Wingo23_weights.txt")); m_PWAS = as.data.frame(m_PWAS); print("Male Primary Discovery loaded")
...
a$Female.GWAS.SNP = ifelse(is.na(a$Female.PD.GWAS.SNP), a$Female.SD.GWAS.SNP, a$Female.PD.GWAS.SNP)

# add sex-strat GWAS stats for each GWAS.SNP
a$Female.GWAS.Beta = ifelse(a$Female.GWAS.SNP %in% f_stats$SNP, f_stats$BETA[match(a$Female.GWAS.SNP, f_stats$SNP)], NA)
a$Female.GWAS.SE = ifelse(a$Female.GWAS.SNP %in% f_stats$SNP, f_stats$SE[match(a$Female.GWAS.SNP, f_stats$SNP)], NA)
a$Female.GWAS.P = ifelse(a$Female.GWAS.SNP %in% f_stats$SNP, f_stats$P[match(a$Female.GWAS.SNP, f_stats$SNP)], NA)

a$Male.GWAS.Beta = ifelse(a$Female.GWAS.SNP %in% m_stats$SNP, m_stats$BETA[match(a$Female.GWAS.SNP, m_stats$SNP)], NA)
a$Male.GWAS.SE = ifelse(a$Female.GWAS.SNP %in% m_stats$SNP, m_stats$SE[match(a$Female.GWAS.SNP, m_stats$SNP)], NA)
a$Male.GWAS.P = ifelse(a$Female.GWAS.SNP %in% m_stats$SNP, m_stats$P[match(a$Female.GWAS.SNP, m_stats$SNP)], NA)
