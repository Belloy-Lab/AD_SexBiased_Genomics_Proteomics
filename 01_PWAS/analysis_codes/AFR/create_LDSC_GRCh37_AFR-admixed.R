################################################################# Danielle Reid
##################################################################### 05/28/24
###create a LD ref panel for AFR-admixed ancestry from 1000G build 37
...
Fil_1000G <- "1000g_AFR-admixed"

# 1KG output folder
tDir_1000G <- paste(tDir,"1KG_AFR-admixed/",sep="")

# genetic map file
hg19map_fold <- "/storage1/fs1/belloy/Active/02_Data/01_Incoming/1000G_Grch37/genetic_map_b37/"

################################################################################ 
# identify AFR-admixed subjects in 1KG
...
dem_1000G <- dem_1000G[which(dem_1000G$sPOP=="AFR"| dem_1000G$sPOP=="EUR" | dem_1000G$sPOP=="AMR"),1:2]
names(dem_1000G)[2] <- "#IID"
dem_1000G <- dplyr::select(dem_1000G, -SID)
write.table(dem_1000G,paste(tDir_1000G,"1K_AFR-admixed_IDs.txt",sep=""),
            row.names = F, col.names = F, quote = F
)

################################################################################ 
## Extract AFR, EUR, and AMR subjects from 1KG
...
arg <- paste(opt$PATH_plink2, 
                 ...
                 "--keep",paste(tDir_1000G,"1K_AFR-admixed_IDs.txt",sep=""),
                 "--allow-no-sex",
