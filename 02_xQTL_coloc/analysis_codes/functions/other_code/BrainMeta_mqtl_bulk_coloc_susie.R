### Preprocessing/Analysis code for Autosomal QTL analysis (BrainMeta mQTL SuSiE)
...
for (i in 1:nrow(all_suggestive_snps)) {
  ...
  merged_updated <- merged_updated %>%
    ...
    dplyr::rename(CHR = CHR.x, BP = BP.x, ALLELE1 = ALLELE1.x, ALLELE0 = ALLELE0.x, AD_P = P, AD_EAF = A1FREQ, 
                  ...
                  QTL_P = p, QTL_BETA = BETA.x, QTL_SE = SE.x, QTL_EAF = Freq, QTL_N = N, CpG = Probe, CpGpos = Probe_bp) %>% 
    dplyr::select(CHR, BP, SNP, ALLELE1, ALLELE0, AD_EAF, AD_BETA, AD_SE, Z, AD_P, AD_N,
                  QTL_EAF, QTL_BETA, QTL_SE, QTL_P, QTL_N, CpG, CpGpos, s)
  ...
  source("/storage2/fs1/belloy2/Active/04_Code/$USER/xQTL/functions/autosome_mQTL_function_susie.R")  
  
  # Run caQTL colocalization
  print(paste("Starting analysis for locus", locus_index, "in tissue:", tissue, sep = " "))
  
  result <- mQTL_colocalization(merged, all_suggestive_snps, name_dset, out_dir)
