### Preprocessing/Analysis code for Autosomal QTL analysis (BrainMeta caQTL SuSiE)
...
for (i in 1:nrow(all_suggestive_snps)) {
  ...
  df_2 = df %>% 
    ...
    mutate(SE = Beta/Z_score_fixed,
           CHR = chrom,
           p_value_log = 2 * pnorm(abs(Z_score_fixed), lower.tail = FALSE, log.p = TRUE),
           p_value = 2*pnorm(-abs(Z_score_fixed))) %>% 
    filter(p_value != 0)
  
  
  
  ## Merge QTL file and AD File by Variant
  merged_updated = merge(df_2, ad_df2, by.x = "Variant", by.y = "SNP") 
  print(paste("There are", n_distinct(merged_updated$Variant), "in first merge", sep = " "))
  
  
  # Create appropriate variable names for efficient QTL colocalization
  merged_updated <- merged_updated %>%
    dplyr::rename(CHR = CHR.x, AD_P = P, AD_EAF = A1FREQ, AD_BETA = BETA, AD_SE = SE.y, AD_N = N_incl,
                  QTL_P = p_value, QTL_BETA = Beta, QTL_SE = SE.x, PeakPos = caQTL_order, SNP = Variant) %>% 
    mutate(QTL_N = 206,
           QTL_EAF = AD_EAF) %>% 
  ...
  source("/storage2/fs1/belloy2/Active/04_Code/$USER/xQTL/functions/autosome_caQTL_function_susie.R")  
